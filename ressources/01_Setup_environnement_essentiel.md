...
> ⚠️ **Tu as déjà Anaconda / Miniconda installé ?** (fréquent sous Windows.)
> Ne fais **pas** cohabiter deux gestionnaires dans le même projet. Deux options propres :
> - **Tout conda** : `conda create -n dev-id python=3.11` puis `conda activate dev-id`,
>   et installe les libs avec `pip install -r requirements.txt` **dans cet env conda**
>   (pas de `conda install` + `pip` désordonné sur les mêmes paquets).
> - **Sans conda** : lance `conda deactivate` (parfois 2×, jusqu'à ne plus voir `(base)`
>   dans ton terminal) **avant** de créer un env `uv`/`venv`.
>
> Le piège classique : laisser l'env `(base)` de conda actif et installer par-dessus avec
> pip → conflits de versions difficiles à diagnostiquer. Un projet = **un seul** env, **un seul** gestionnaire.
...
1. Crée un dossier `dev-id-onboarding-<prenom>` quelque part de propre (pas le bureau).
...
6. Crée un fichier `.gitignore` avec au minimum :
   ...
   *.pyc
   ...
   .DS_Store
   ```
7. Crée un repo `dev-id-onboarding-<prenom>` sur GitHub (public ou privé avec accès en
   lecture pour la formatrice).
8. Lie le repo local au remote, fais un premier commit, pousse :
   ```bash
   git remote add origin git@github.com:<ton_user>/dev-id-onboarding-<prenom>.git
   git add .gitignore
